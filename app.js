...
// Comparison View Split-Screen Logic
function initializeComparisonView() {
  // Populate shared exam selection
  populateComparisonExamSelect();

  // Room config selectors sync
  const comparisonRoom = document.getElementById('comparison-room-config');
  const normalRoom = document.getElementById('normal-room-config');
  const pandemicRoom = document.getElementById('pandemic-room-config-comparison');
  if (comparisonRoom && normalRoom && pandemicRoom) {
    comparisonRoom.addEventListener('change', e => {
      normalRoom.value = e.target.value;
      pandemicRoom.value = e.target.value;
    });
    normalRoom.addEventListener('change', e => {
      comparisonRoom.value = e.target.value;
      pandemicRoom.value = e.target.value;
    });
    pandemicRoom.addEventListener('change', e => {
      comparisonRoom.value = e.target.value;
      normalRoom.value = e.target.value;
    });
  }
  ...
  // Exam selection sync
  const examSelect = document.getElementById('comparison-exam-select');
  if (examSelect) {
    examSelect.addEventListener('change', () => runBothAllocations());
  }

  // Special needs selectors
  const normalSN = document.getElementById('normal-special-needs');
  const pandemicSN = document.getElementById('pandemic-special-needs-comparison');
  if (normalSN && pandemicSN) {
    normalSN.addEventListener('change', () => runBothAllocations());
    pandemicSN.addEventListener('change', () => runBothAllocations());
  }

  // Distance slider for pandemic (only pandemic side)
  const pandemicSlider = document.getElementById('pandemic-distance-slider');
  const pandemicDistanceValue = document.getElementById('pandemic-distance-value');
  if (pandemicSlider) {
    pandemicSlider.value = appState.pandemicDistance;
    pandemicDistanceValue.textContent = appState.pandemicDistance.toFixed(1) + 'm';
    pandemicSlider.addEventListener('input', e => {
      const dist = Number(e.target.value);
      appState.pandemicDistance = dist;
      pandemicDistanceValue.textContent = dist.toFixed(1) + 'm';
      document.getElementById('pandemic-distance').textContent = dist.toFixed(1) + 'm';
      runBothAllocations(); // Dynamic metric update
    });
  }

  // Metric display
  document.getElementById('normal-distance').textContent = '2.0m';
  document.getElementById('pandemic-distance').textContent = appState.pandemicDistance.toFixed(1) + 'm';

  // Allocation run buttons
  document.querySelector('.btn[onclick="runNormalAllocation()"]')?.addEventListener('click', runNormalAllocation);
  document.querySelector('.btn[onclick="runPandemicAllocationComparison()"]')?.addEventListener('click', runPandemicAllocationComparison);
}

function runNormalAllocation() {
  // Use exam, room config, and special needs from panel
  const examId = document.getElementById('comparison-exam-select').value;
  const room = document.getElementById('normal-room-config').value || 'standard';
  const prioritizeSN = document.getElementById('normal-special-needs').checked;
  if (!examId) {
    showToast('Please select an exam', 'error');
    return;
  }
  ...
  const normalResult = runDistanceBasedAllocation(eligibleStudents, config, 2, 'normal', prioritizeSN);
  appState.normalAllocation = normalResult;
  displayAllocation(normalResult, 'normal', config);
  maybeEnableExports();
}

function runPandemicAllocationComparison() {
  const examId = document.getElementById('comparison-exam-select').value;
  const room = document.getElementById('pandemic-room-config-comparison').value || 'standard';
  const prioritizeSN = document.getElementById('pandemic-special-needs-comparison').checked;
  if (!examId) {
    showToast('Please select an exam', 'error');
    return;
  }
  ...
  if (!exam) return;
  const config = ROOM_CONFIGS[room];
  const dist = appState.pandemicDistance;
  const eligibleStudents = appState.students.filter(s => 
    s.subjects.some(sub => sub.toLowerCase() === exam.subject.toLowerCase())
  );
  const pandemicResult = runDistanceBasedAllocation(eligibleStudents, config, dist, 'pandemic', prioritizeSN);
...
function runBothAllocations() {
  runNormalAllocation();
  runPandemicAllocationComparison();
  // Show insights if both successful
  if (appState.normalAllocation && appState.pandemicAllocation) {
    const eligibleStudents = appState.normalAllocation.metrics.totalStudents;
    displayComparisonInsights(appState.normalAllocation, appState.pandemicAllocation, eligibleStudents);
  }
}

function exportNormalAllocation() {
  if (!appState.normalAllocation) {
    showToast('No normal allocation to export', 'error');
    return;
  }
  const result = appState.normalAllocation;
  let csvContent = 'Row,Column,Position,Student ID,Name,Department,Year,Special Needs,Distance Setting\n';
  result.allocated.forEach(seat => {
    csvContent += `${seat.row + 1},${seat.col + 1},${seat.position},${seat.student.id},${seat.student.name},${seat.student.department},${seat.student.year},${seat.student.specialNeeds},2.0m\n`;
  });
  const blob = new Blob([csvContent], { type: 'text/csv' });
  const url = window.URL.createObjectURL(blob);
  const a = document.createElement('a');
  a.href = url;
  a.download = `normal_allocation_2m_${Date.now()}.csv`;
  a.click();
  window.URL.revokeObjectURL(url);
  showToast('Normal allocation exported', 'success');
}

function exportPandemicAllocationComparison() {
  if (!appState.pandemicAllocation) {
    showToast('No pandemic allocation to export', 'error');
    return;
  }
  const result = appState.pandemicAllocation;
  let csvContent = 'Row,Column,Position,Student ID,Name,Department,Year,Special Needs,Distance Setting\n';
  ...
  a.click();
  window.URL.revokeObjectURL(url);
  showToast('Pandemic allocation exported', 'success');
}

function exportComparisonReport() {
  // For demonstration, just export metrics as CSV
  if (!appState.normalAllocation || !appState.pandemicAllocation) {
    showToast('Run both allocations first', 'error');
    return;
  }
  let csv = 'Metric,Normal,Pandemic\n';
  const n = appState.normalAllocation.metrics;
  const p = appState.pandemicAllocation.metrics;
  csv += `Students Allocated,${n.studentsAllocated},${p.studentsAllocated}\n`;
  csv += `Avg Distance,${n.avgDistance},${p.avgDistance}\n`;
  csv += `Efficiency,${n.efficiency}%,${p.efficiency}%\n`;
  ...
  a.click();
  window.URL.revokeObjectURL(url);
  showToast('Exported comparison report', 'success');
}

function maybeEnableExports() {
  document.getElementById('export-normal-btn').disabled = !appState.normalAllocation;
  document.getElementById('export-pandemic-comparison-btn').disabled = !appState.pandemicAllocation;
}

// Old comparison view functions maintained for legacy views
function oldInitializeComparisonView() {
  // Initialize distance sliders for old comparison view (if exists)
...
function oldRunBothAllocations() {
  const examId = document.getElementById('comparison-exam-select').value;
